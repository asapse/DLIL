...
foreach $balise (qw/trans speaker topic episode section turn sync background event comment who transcription/) {
    $proceed{$balise}=1; # si mis à 0, les balises à 0 ne sont pas regardées
}
$outputformat="stm";
$norm{speaker}=0;
...
if ($outputformat) { # prise en compte du format des fichiers de sortie
    &initformat(); # initialisation du format (stm, lexh, text, ...) cette initialisation peut contenir des paramètres particuliers (-i, -c)
}


# prise en compte des elements a supprimer ou ignorer
if ($ignorecontent) { # initialisation des patterns de commentaires et events à supprimer (suppression du contenu de l'event)
...
if ($ignoresegment) { # initialisation des patterns de commentaires et events à supprimer (suppression des segment content l'event)
...
if ($ignoretag) { # initialisation des patterns de commenttaires et events à supprimer (suppression des balises de l'event)
...
if ($extract) { # initialisation des patterns de commentaires et events à extraire exclusivement
...
if ($outputformat =~ /^stm(\-ne)?$/i || $outputformat eq "lex") {
    ...
    # remplacement des balises next et previous, un peu difficile à traiter autrement 
...
#    }
   ...
   while ($file_content=~s/\n(\s*[^<][^\n]*\n<event\s+[^>]*extent=\"previous\"[^>]*\/>)/\n@@@@/si) {
    ...
    #supprime les balises previous restant (précédées d'une autre balise event = balise non conventionnelle
...
sub flush_episode() { # procedure d'impression de l'épisode dans le fichier de sortie
...
#	$format{overlappingspeakers_transcription}="";
#	$format{overlappingspeakers_speaker}="inter_segment_gap";
...
#	    $ignoretag=";
	    $format{event_begin}="<entities:%event_desc ";
	    $format{event}="%event_desc";
	    $format{event_end}=" >";
...
sub acoustic_conditions() { # traitement des conditions acoustiques
	...
	if ($overlappingbackground ne "") {
	    ...
	    if ($overlappingbackground =~ /off$/ ) { # cas d'un overlapping background à terminer à l'impression.
		$overlappingbackground="";
	    }
	}
	if ($dialect{$speaker_turn} eq "nonnative") {
...
#-------------- fin des procedures utilisées pour le formattage
...
=over 8
=item B<-Nouveautés version 0.74>
    - complément débogage balises pronounce

=item B<-Nouveautés version 0.73>
    - amélioration format stm-ne et ctm-ne pour évaluation des entités nommées
    - prise en compte des balises pronounce "(.*:) prononciation + (1[1-9] cent...)
    - format lex amélioré

=item B<-Nouveautés version 0.72>

    - correction des bugs sur les balises extent="next" par prétraitement systématique

=item B<-Nouveautés version 0.72>

    - ajout des formats de sortie trs, mtdm, stm-ne 
    - modification des règles de conditions acoustiques (suite au changement de Transcriber 1.4.7)
    - ajout des support des balises pronounce spécifiques (19 cent... / URL)
    - inversion de la commande k (keep audiofilename au lieu de keep filename)
    - correction de la fonctionnalité de postprocessing. fonction aussi bien avec STDOUT qu'avec la sortie
      en fichiers (e.g. parsetrs file.trs -p 'normalize'
    - ajout du comptage du temps pris par le programme à la sorite (en mode verbose)

=item B<-Nouveautés version 0.5 :>

    - possibilité de supprimer certaines balises à la volée,
      ou au contraire de n'extraire que ces balises (utile pour
      entites nommées par exemple) (options -extract/-remove)

=item B<Nouveautés version 0.6 :>

    - conversion stm rendue plus compatible avec l'export transcriber 1.4.6 :
               - support des noms de locuteurs "global" ou non
               - gestion des conditions acoustiques bruitées
               - multilocuteurs dans excluded_regions
    le support des tours à deux locuteurs aboutit toujours à des 
    "excluded regions"

=item B<Nouveautés version 0.61 :>

    - format de sortie par défaut : stm au lieu de lexh
    - gère 3 types d'ignore pour les balises : celles qu'on ne veut
    ...
    - conversion stm : 
               - correction des incompatibilités type "inter_segment_gap"
               - gestion des balises previous et next compatible (transformées
                 en balises begin & end)
               - correction pour prise en compte du mode f1 (spontaneous)
               - résolution du problème de bords pour les balises background en 
                 début/fin de tour.
    ...
    - différences persistant avec l'export stm transcriber 1.4.6:
               - split des segments comportant une balise background en deux
                 (ou plusieurs) segments de conditions acoustiques différentes
               - après un tour à deux locuteurs, l'export stm de transcriber
                 force la condition acoustique du tour suivante à f3, ce qui
                 n'est pas le cas ici
               - transcriber ignore les balises next
               - transcriber ne gère pas les balises previous en début de turn
               - gestion différentes du cas limite "mot [noise] [+noise]"
               - transcriber ne supporte pas les balises débordant d'une section
    - mise à jour du manuel

=item B<Nouveautés version 0.62 :>
